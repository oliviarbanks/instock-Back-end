...
exports.updateInventories = (req, res) => {
  ...
  knex("inventories")
    ...
    .then(() => {
      res
        .status(200)
        .send(`Inventories with id: ${req.params.id} has been updated`);
    })
    .catch((err) =>
      res.status(400).send(`Error updating inventories ${req.params.id} ${err}`)
    );
};
...
exports.addInventory = (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .send(
        "Please make sure to provide warehouse ID, item name, description, category, sPOST/CREATEtatus, and quantity fields in a request."
      );
  }
